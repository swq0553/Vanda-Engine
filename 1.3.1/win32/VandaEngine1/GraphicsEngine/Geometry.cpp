...
CBool CPolyGroup::SetShadowMap( const CChar * path )
{ 
	...
	if( tempImg == NULL )
	{
		...
		sprintf( m_strTemp3, "%s%s%s%s", "assets/vscenes/", g_currentVSceneNameWithoutDot, "/Textures/", m_strTemp2 );
...
CBool CPolyGroup::SetAlphaMap( const CChar * path )
{ 
	CChar m_strTemp[MAX_NAME_SIZE];
	Cpy( m_strTemp, path);
...
{ 
	...
	if( m_normalMapImg == NULL )
	{
		...
		if(!CTexture::LoadDDSTexture( m_normalMapImg, m_strNormalMap, NULL ) )
			MessageBox( NULL, _T(" CGeometry::SetShadowMap>Couldn't load the Normal map"), _T("VandaEngine Error"), MB_OK );
...
CVoid CGeometry::SetAlphaMap( const CChar * path )
{
	Cpy( m_strAlphaMap, path );
	...
	if( m_alphaMapImg == NULL )
	{
		...
		Cpy( g_currentVSceneNameWithoutDot, g_currentVSceneName );
		GetWithoutDot( g_currentVSceneNameWithoutDot );
		sprintf( m_strAlphaMap, "%s%s%s%s", "assets/vscenes/", g_currentVSceneNameWithoutDot, "/Textures/", m_strTemp );
		if(! CTexture::LoadDDSTexture( m_alphaMapImg, m_strAlphaMap, NULL ) )
			MessageBox( NULL, _T(" CGeometry::SetShadowMap>Couldn't load the Alpha map"), _T("VandaEngine Error"), MB_OK );
...
CVoid CGeometry::SetShadowMap( const CChar * path ) { 
	Cpy( m_strShadowMap, path );
	...
	if( m_shadowMapImg == NULL )
	{
		...
		Cpy( g_currentVSceneNameWithoutDot, g_currentVSceneName );
		GetWithoutDot( g_currentVSceneNameWithoutDot );
		sprintf( m_strShadowMap, "%s%s%s%s", "assets/vscenes/", g_currentVSceneNameWithoutDot, "/Textures/", m_strTemp );

		if( !CTexture::LoadDDSTexture( m_shadowMapImg, m_strShadowMap, NULL ) )
			MessageBox( NULL, _T(" CGeometry::SetShadowMap>Couldn't load the shadow map"), _T("VandaEngine Error"), MB_OK );
...
CVoid CGeometry::SetGlossMap( const CChar * path ) {
	...
	if( m_glossMapImg == NULL )
	{
		...
		if(!CTexture::LoadDDSTexture( m_glossMapImg, m_strGlossMap, NULL ) )
			MessageBox( NULL, _T(" CGeometry::SetShadowMap>Couldn't load the Gloss map"), _T("VandaEngine Error"), MB_OK );
...
CVoid CGeometry::SetHeightMap( const CChar * path ) {
	...
	if( m_heightMapImg == NULL )
	{
		...
		if(!CTexture::LoadDDSTexture( m_heightMapImg, m_strHeightMap, NULL ) )
			MessageBox( NULL, _T(" CGeometry::SetShadowMap>Couldn't load the height map"), _T("VandaEngine Error"), MB_OK );
...
CVoid CGeometry::SetGroupRender( CPolyGroup* group )
{
	...
	if( g_render.UsingShader() && g_render.m_useShader && g_options.m_enableShader)
	{
		...
		else if( group->m_hasDiffuse && group->m_diffuseImg->GetFormat() == 4 )
		{
		...
		if( group->m_hasShadowMap && m_texCoords[1] )
		{
