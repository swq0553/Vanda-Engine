...
{
public:
	CInstanceGeometry() : m_abstractGeometry(0) { m_renderCount = 0; m_nameIndex = -1; m_hasPhysX = CFalse; Cpy(m_physXName, "\n"); m_lodAlgorithm = eLOD_NONE; m_physXDensity = 0.0f; m_physXPercentage = 50; m_physXCount = 0; m_firstUpdate = CTrue; m_isTrigger = CFalse;Cpy( m_enterScript, "\n"); Cpy( m_exitScript, "\n" );
	m_updateEnterScript = m_updateExitScript = CTrue; m_hasEnterScript = m_hasExitScript = CFalse; m_distanceFromCamera = 0.0f;}
...
public:
	...
	CChar m_strOriginalShadowMapPath[MAX_NAME_SIZE]; //save functions
	CImage *m_shadowMapImg;
	CBool SetShadowMap( CString path, CBool test = CFalse );
	CBool m_hasShadowMap;
	CBool m_loadedShadowMap;
	CBool m_updateShadowMap;

	CChar m_strAlphaMap[MAX_NAME_SIZE];
	CChar m_strOriginalAlphaMapPath[MAX_NAME_SIZE]; //save functions
	CImage *m_alphaMapImg;
	CBool SetAlphaMap( CString path, CBool test = CFalse );
	CBool m_hasAlphaMap;
	CBool m_loadedAlphaMap;
	CBool m_updateAlphaMap;
...
protected:
	...
	CBool SetupAlphaTexture(CNode *parentNode, CInstance * instance);
	CBool SetupNormalTexture(CNode *parentNode, CInstance * instance);
	CBool SetupShadowTexture(CNode *parentNode, CInstance * instance);
...
public:
	...
	CGeometry()
	{
		...
		m_hasAlphaMap = m_hasGlossMap = m_hasNormalMap = m_hasHeightMap = m_hasDuDvMap = m_hasShadowMap = m_hasDirtMap = m_hasDiffuse = CFalse;
		m_cullFaces = CTrue;
		...
		//m_shadowMapImg = new CImage();
		//m_dirtMapImg = new CImage();
		//m_alphaMapImg = new CImage();
	...
	CVoid SetAlphaMap( CString path/*, CBool test = CFalse*/ );
	CVoid SetDirtMap( CString path );
	CVoid SetShadowMap( CString path );
...
protected:
	...
	CVoid SetupAlphaTexture(CNode *parentNode, CInstance * instance)
	{
		CBool m_setGeoName = CFalse;
		for (CUInt i = 0; i < m_groups.size(); i++ )
		{
			if( m_groups[i]->SetupAlphaTexture( parentNode, instance ) )
				m_setGeoName = CTrue;
		...
		else
		{
			...
			for( CUInt i = 0; i < m_groups.size(); i++ )
			{
				...
				{
					if( !Cmp( m_groups[i]->m_strAlphaMap, m_groups[j]->m_strAlphaMap ) ) //if these materials are not equal
					{
						m_foundTarget = CTrue;
					}
					if( m_foundTarget )
			...
			if( m_foundTarget )
			{
				Cpy( m_strAlphaMap, "*Different Textures*" );
				m_hasAlphaMap = CTrue;
			}
			else //same materials
			{
				Cpy( m_strAlphaMap, m_groups[0]->m_strAlphaMap );
				m_hasAlphaMap = CTrue;
			}
		}
	}
	...
	CVoid SetupShadowTexture(CNode *parentNode, CInstance * instance)
	{
		CBool m_setGeoName = CFalse;
		for (CUInt i = 0; i < m_groups.size(); i++ )
		{
			if( m_groups[i]->SetupShadowTexture( parentNode, instance ) )
				m_setGeoName = CTrue;
		...
		else
		{
			...
			for( CUInt i = 0; i < m_groups.size(); i++ )
			{
				for( CUInt j = 0; j < m_groups.size(); j++ )
				{
					if( !Cmp( m_groups[i]->m_strShadowMap, m_groups[j]->m_strShadowMap ) ) //if these materials are not equal
					{
						m_foundTarget = CTrue;
					}
					if( m_foundTarget )
			...
			if( m_foundTarget )
			{
				Cpy( m_strShadowMap, "*Different Textures*" );
				m_hasShadowMap = CTrue;
			}
			else //same materials
			{
				Cpy( m_strShadowMap, m_groups[0]->m_strShadowMap );
				m_hasShadowMap = CTrue;
			}
		}
	}
