...
public:
	...
	CBool Load( CChar * fileName, CBool reportWarningsAndErrors = CTrue, CBool readFromBuffer = CFalse );
	CVoid Destroy();
	...
	CVoid RenderAnimatedModels(CBool sceneManager);
	CVoid RenderModelsControlledByPhysX(CBool sceneManager);
	CVoid RenderForWater(CWater* water);
	...
	CVoid Update( CFloat elapsedTime = 0.0f );
	CVoid UpdateBlendCycleList(CFloat elapsedTime);
	...
	CVoid SetupAlphaTexture();
	CVoid SetupNormalTexture();
	CVoid SetupShadowTexture();
	...
	CInt GeneratePhysX( CPhysXAlgorithm algorithm, CFloat density, CInt percentage, CBool isTrigger, CInstanceGeometry* m_instanceGeo)
	{
		...
		{
			if( m_instanceGeo->m_lodAlgorithm == eLOD_LENGTH_CURVATURE ||
				m_instanceGeo->m_lodAlgorithm == eLOD_LENGTH )
			{
				if( m_instanceGeo->m_lodAlgorithm != algorithm ) //regenerate PhysX informations
				{
					PrintInfo( "\nRegenerating LOD..." );
					m_instanceGeo->m_abstractGeometry->CalculateLODInfo(algorithm);
				}
	...
	CInt GeneratePhysX( CPhysXAlgorithm algorithm, CFloat density, CInt percentage, CBool isTrigger )
	{
		if( g_selectedName != -1 )
		{
			for( CUInt geoCounter = 0; geoCounter < m_geometries.size(); geoCounter++ )
				...
				for( CUInt j = 0; j < m_geo->m_groups.size(); j++ )
				{
					if( g_menu.m_geometryBasedSelection )
					{
						if( selected )
						{
							...
							{
								IsInVSceneList(g_allVSceneNames[index].c_str(), CFalse, CTrue );
							}
						}
					}
	...
	inline CBool CScene::SetAlphaMap( const CChar * path )
	{
		if( g_selectedName != -1 )
		{
			CBool m_setGeoName = CFalse;
			for( CUInt i = 0; i < m_instanceGeometries.size(); i++ )
			{
				...
				for( CUInt j = 0; j < m_geo->m_groups.size(); j++ )
				{
					if( g_menu.m_geometryBasedSelection )
					{
						if( selected )
						{
							if( !m_geo->m_groups[j]->SetAlphaMap( path ) )
								return CFalse;
							CChar temp[MAX_NAME_SIZE];
							Cpy( temp, path );
							GetAfterPath( temp );
							GetWithoutDot( temp );
							Cpy( m_geo->m_strAlphaMap, temp );
							m_geo->m_hasAlphaMap = CTrue;
			...
			if( !g_menu.m_geometryBasedSelection )
			{
				...
				if( m_foundGeo )
				{
					for( CUInt i = 0; i < tempGeo->m_groups.size(); i++ )
					{
						for( CUInt j = 0; j < tempGeo->m_groups.size(); j++ )
						{
							if( !Cmp( tempGeo->m_groups[i]->m_strAlphaMap, tempGeo->m_groups[j]->m_strAlphaMap ) ) //if these materials are not equal
							{
								m_foundTarget = CTrue;
							}
							if( m_foundTarget )
					...
					if( m_foundTarget )
					{
						Cpy( tempGeo->m_strAlphaMap, "*Different Textures*" );
						tempGeo->m_hasAlphaMap = CTrue;
					}
					else //same materials
					{
						Cpy( tempGeo->m_strAlphaMap, tempGeo->m_groups[0]->m_strAlphaMap );
						tempGeo->m_hasAlphaMap = CTrue;
					}
				}
			}
	...
	inline CBool CScene::SetShadowMap( CString path )
	{
		if( g_selectedName != -1 )
		{
			CBool m_setGeoName = CFalse;
				...
				for( CUInt j = 0; j < m_geo->m_groups.size(); j++ )
				{
					if( g_menu.m_geometryBasedSelection )
					{
						if( selected )
						{
							if( m_geometries[i]->m_texCoords[1] )
							{
								if( !m_geo->m_groups[j]->SetShadowMap( path ) )
									return CFalse;
								CChar temp[MAX_NAME_SIZE];
								Cpy( temp, path );
								GetAfterPath( temp );
								GetWithoutDot( temp );
								Cpy( m_geo->m_strShadowMap, temp );
								m_geo->m_hasShadowMap = CTrue;
							...
							{
								MessageBox( NULL, "In order to set shadow map, you must export UV set 2 for COLLADA model", "VandaEngine Error", MB_OK | MB_ICONERROR);
								return CFalse;
							}
						}
					}
					else //material based selection 
					{
						if( m_geo->m_groups[j]->m_nameIndex == g_selectedName )
						{
							if( m_geometries[i]->m_texCoords[1] )
							{
								...
								MessageBox( NULL, "In order to use shadow map, you must export UV set 2 for COLLADA model", "VandaEngine Error", MB_OK | MB_ICONERROR);
						}
					}
				}
			}
			...
			if( !g_menu.m_geometryBasedSelection )
			{
				...
				if( m_foundGeo )
				{
					for( CUInt i = 0; i < tempGeo->m_groups.size(); i++ )
					{
						for( CUInt j = 0; j < tempGeo->m_groups.size(); j++ )
						{
							if( !Cmp( tempGeo->m_groups[i]->m_strShadowMap, tempGeo->m_groups[j]->m_strShadowMap ) ) //if these materials are not equal
							{
								m_foundTarget = CTrue;
							}
							if( m_foundTarget )
					...
					if( m_foundTarget )
					{
						Cpy( tempGeo->m_strShadowMap, "*Different Textures*" );
						tempGeo->m_hasShadowMap = CTrue;
					}
					else //same materials
					{
						Cpy( tempGeo->m_strShadowMap, tempGeo->m_groups[0]->m_strShadowMap );
						tempGeo->m_hasShadowMap = CTrue;
					}
				}
			}
	...
	inline CVoid CScene::RemoveShadowMap()
	{
		if( g_selectedName != -1 )
		{
			if( g_menu.m_geometryBasedSelection )
			{
				for( CUInt i = 0; i < m_instanceGeometries.size(); i++ )
				{
					...
					if( selected )
					{
						for( CUInt j = 0; j < m_instanceGeometries[i]->m_abstractGeometry->m_groups.size(); j++ )
						{
							m_instanceGeometries[i]->m_abstractGeometry->RemoveShadowMap();
							m_instanceGeometries[i]->m_abstractGeometry->m_groups[j]->RemoveShadowMap();
						}
					}
				}
			}
			else //material based selection
			{
				for( CUInt i = 0; i < m_geometries.size(); i++ )
				{
					...
					{
						if( m_geometries[i]->m_groups[j]->m_nameIndex == g_selectedName )
							m_geometries[i]->m_groups[j]->RemoveShadowMap();
					}
				} 
				CUInt numGroups = 0;
				CGeometry* m_geo = NULL;
				...
				for( CUInt i = 0; i < m_geometries.size(); i++ )
				{
					for( CUInt j = 0; j < m_geometries[i]->m_groups.size(); j++ )
					{
						if( m_geometries[i]->m_groups[j]->m_nameIndex == g_selectedName )
						{
							...
							for( CUInt k = 0; k < m_geometries[i]->m_groups.size(); k++ )
							{
								if( !m_geometries[i]->m_groups[k]->m_hasShadowMap )
								{
									numGroups++;
								}
							}
				...
				if( foundGeo )
				{
					if ( CUInt( m_geo->m_groups.size() ) == numGroups )
					{
					m_geo->RemoveShadowMap();
					}
					else
					{
						Cpy( m_geo->m_strShadowMap, "*Different Textures*" );
						m_geo->m_hasShadowMap = CTrue;
					}
				}
			}
	...
	inline CVoid CScene::RemoveAlphaMap()
	{
		if( g_selectedName != -1 )
		{
			if( g_menu.m_geometryBasedSelection )
			{
				for( CUInt i = 0; i < m_instanceGeometries.size(); i++ )
				{
					...
					if( selected )
					{
						for( CUInt j = 0; j < m_instanceGeometries[i]->m_abstractGeometry->m_groups.size(); j++ )
						{
							m_instanceGeometries[i]->m_abstractGeometry->RemoveAlphaMap();
							m_instanceGeometries[i]->m_abstractGeometry->m_groups[j]->RemoveAlphaMap();
						}
					}
				}
			}
			else //material based selection
			{
				for( CUInt i = 0; i < m_geometries.size(); i++ )
				{
					...
					{
						if( m_geometries[i]->m_groups[j]->m_nameIndex == g_selectedName )
							m_geometries[i]->m_groups[j]->RemoveAlphaMap();
					}
				} 
				CUInt numGroups = 0;
				CGeometry* m_geo = NULL;
				...
				for( CUInt i = 0; i < m_geometries.size(); i++ )
				{
					for( CUInt j = 0; j < m_geometries[i]->m_groups.size(); j++ )
					{
						if( m_geometries[i]->m_groups[j]->m_nameIndex == g_selectedName )
						{
							...
							for( CUInt k = 0; k < m_geometries[i]->m_groups.size(); k++ )
							{
								if( !m_geometries[i]->m_groups[k]->m_hasAlphaMap )
								{
									numGroups++;
								}
							}
				...
				if( foundGeo )
				{
					if ( CUInt( m_geo->m_groups.size() ) == numGroups )
					{
					m_geo->RemoveAlphaMap();
					}
					else
					{
						Cpy( m_geo->m_strAlphaMap, "*Different Textures*" );
						m_geo->m_hasAlphaMap = CTrue;
					}
				}
			}
...
private:
	...
	CBool ReadAnimationLibrary( domLibrary_animationsRef lib );
	CAnimation *ReadAnimation( domAnimationRef lib );
