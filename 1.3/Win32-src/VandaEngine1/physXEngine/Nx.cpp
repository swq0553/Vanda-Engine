...
CBool CNovodex::initNx( CFloat XCharacterPos, CFloat YCharacterPos, CFloat ZCharacterPos, CFloat gravity, CFloat crtlRadius, CFloat crtlHeight, CBool createScene )
{
	if( createScene )
	{
		gPhysXscene = NULL;
	...
	//Default gravity
	gDefaultGravity.x = 0.0f;
	gDefaultGravity.y = -9.8f;
	gDefaultGravity.z = 0.0f;
	wireFrame = false;
	zero = NxVec3(0,0,0);
	...
	gCharacterSpeed = (CFloat)DEFAULT_CHARACTER_SPEED;
	bFixedStep = false;
	gDesiredDistance = 2.f;
	G = -98.1f;
	...
	InitCharacterControllers( XCharacterPos, YCharacterPos, ZCharacterPos, crtlRadius, crtlHeight);	
...
NxActor* CNovodex::CreateGroundPlane()
{
    // Create a plane with default descriptor
    NxPlaneShapeDesc planeDesc;
    NxActorDesc actorDesc;
	actorDesc.shapes.pushBack(&planeDesc);
    return gPhysXscene->createActor(actorDesc);
}
...
CVoid CNovodex::InitCharacterControllers( CFloat XPos, CFloat YPos, CFloat ZPos, CFloat radius, CFloat height)
{
	gCM = NULL;
	gControllers = NULL;
...
public:
	...
	virtual bool NXU_preNotifyActor(NxActorDesc &actor, const char *userProperties)
	{
		...
		while (nShapes--)
		{
			...
			{
				shapes[nShapes]->setGroup(GROUP_COLLIDABLE_PUSHABLE); 
				//shapes[nShapes]->setSkinWidth( 0.01 );
			}
			else
			{
				// I set the group to GROUP_COLLIDABLE_NON_PUSHABLE, so the character pushes the dynamic objects and can't push the static objects.
