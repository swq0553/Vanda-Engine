...
void CEditSceneOptions::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_ALPHAMAP_NAME, m_editBoxAlphaMapName);
	DDX_Control(pDX, IDC_EDIT_GLOSSMAP_NAME, m_editBoxGlossMapName);
	DDX_Control(pDX, IDC_EDIT_SHADOWMAP_NAME, m_editBoxShadowMapName);
	DDX_Control(pDX, IDC_EDIT_DIRTMAP_NAME, m_editBoxDirtMapName);
	DDX_Control(pDX, IDC_EDIT_MESH_PATH2, m_editBoxBanner);
...
BEGIN_MESSAGE_MAP(CEditSceneOptions, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_LOADING_BANNER, &CEditSceneOptions::OnBnClickedBtnLoadingBanner)
	ON_EN_CHANGE(IDC_EDIT_NORMALMAP_NAME, &CEditSceneOptions::OnEnChangeEditNormalmapName)
	ON_EN_CHANGE(IDC_EDIT_ALPHAMAP_NAME, &CEditSceneOptions::OnEnChangeEditAlphamapName)
	ON_EN_CHANGE(IDC_EDIT_GLOSSMAP_NAME, &CEditSceneOptions::OnEnChangeEditGlossmapName)
	ON_EN_CHANGE(IDC_EDIT_SHADOWMAP_NAME, &CEditSceneOptions::OnEnChangeEditShadowmapName)
...
BOOL CEditSceneOptions::OnInitDialog()
{
	...
	m_strAlphaMapName = g_extraTexturesNamingConventions.m_alphaMapName;
	...
	m_strShadowMapName = g_extraTexturesNamingConventions.m_shadowMapName;
	...
	m_editBoxAlphaMapName.SetWindowTextA(m_strAlphaMapName);
	m_editBoxDirtMapName.SetWindowTextA(m_strDirtMapName);
	m_editBoxShadowMapName.SetWindowTextA(m_strShadowMapName);
...
void CEditSceneOptions::OnOK()
{
	...
	Cpy( g_extraTexturesNamingConventions.m_alphaMapName, (LPCSTR) m_strAlphaMapName );
	...
	Cpy( g_extraTexturesNamingConventions.m_shadowMapName, (LPCSTR) m_strShadowMapName );
	...
	if( setBanner )
	{
		...
		{
			g_sceneBanner.IsInVSceneList( g_allVSceneNames[index], CFalse, CTrue ); //Write to zip file and copy the texture
		}
		//save functions/////////////////////////////////
...
void CEditSceneOptions::OnBnClickedBtnLoadingBanner()
{
	// TODO: Add your control notification handler code here
...
void CEditSceneOptions::OnEnChangeEditAlphamapName()
{
	m_editBoxAlphaMapName.GetWindowTextA(m_strAlphaMapName);
}
...
void CEditSceneOptions::OnEnChangeEditShadowmapName()
{
	m_editBoxShadowMapName.GetWindowTextA(m_strShadowMapName);
}
