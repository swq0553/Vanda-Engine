...
CImage *CScene::ReadImage( domImageRef lib )
{
	...
	CImage * image = GetImage(GetAfterPath( file ) );
...
CBool CScene::ReadAnimationLibrary( domLibrary_animationsRef lib )
{
...
CAnimation *CScene::ReadAnimation( domAnimationRef animation )
{
	if (animation==NULL) return NULL;
	CAnimation* m_animation = GetAnimation(animation->getID(), animation->getDocumentURI()->getURI());
...
CBool CScene::ReadScene( domVisual_sceneRef scene, CUpAxis up )
{
	...
	for ( CUInt i = 0; i < scene->getNode_array().getCount(); i++)
	{
		...
		if (node) m_nodes[node->GetId()] = node;
	}
...
CNode * CScene::ReadNode( domNodeRef node, CNode * parentNode )
{
	...
	for (CUInt i = 0; i < node->getNode_array().getCount(); i++)
	{
		...
		if (readnode) m_nodes[readnode->GetId()] = readnode;
	}
...
{
	if (lib->getId()==NULL) return NULL;
	CGeometry * geometry = GetGeometry(lib->getID(), lib->getDocumentURI()->getURI());
